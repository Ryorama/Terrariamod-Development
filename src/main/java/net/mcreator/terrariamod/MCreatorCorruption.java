...
public class MCreatorCorruption extends Elementsterrariamod.ModElement {
	@ObjectHolder("terrariamod:corruption")
	public static final CustomBiome biome = null;

	public MCreatorCorruption(Elementsterrariamod instance) {
		super(instance, 74);
	}
	...
	public void init(FMLCommonSetupEvent event) {
		...
		BiomeManager.addBiome(BiomeManager.BiomeType.COOL, new BiomeManager.BiomeEntry(biome, 900));
	}
	...
	static class CustomBiome extends Biome {
		public CustomBiome() {
			super(new Biome.Builder()
					...
					.waterColor(-6749953)
					.waterFogColor(-6749953)
					.surfaceBuilder(
							SurfaceBuilder.DEFAULT,
							new SurfaceBuilderConfig(MCreatorCorruptedGrass.block.getDefaultState(), MCreatorTDirt.block.getDefaultState(),
									MCreatorTDirt.block.getDefaultState())));
	...
	static class CustomTreeFeature extends AbstractTreeFeature<NoFeatureConfig> {
		...
		public boolean place(Set<BlockPos> changedBlocks, IWorldGenerationReader worldgen, Random rand, BlockPos position, MutableBoundingBox bbox) {
			...
			if (position.getY() >= 1 && position.getY() + height + 1 <= world.getHeight()) {
				...
				if (!spawnTree) {
					...
					if (!((ground == MCreatorCorruptedGrass.block.getDefaultState().getBlock() || ground == MCreatorTDirt.block.getDefaultState()
							.getBlock()) && (ground2 == MCreatorCorruptedGrass.block.getDefaultState().getBlock() || ground2 == MCreatorTDirt.block
							.getDefaultState().getBlock())))
						return false;
					BlockState state = world.getBlockState(position.down());
					if (position.getY() < world.getHeight() - height - 1) {
						setTreeBlockState(changedBlocks, world, position.down(), MCreatorTDirt.block.getDefaultState(), bbox);
								...
								for (int i2 = position.getZ() - j1; i2 <= position.getZ() + j1; ++i2) {
									...
									if (Math.abs(position.getX()) != j1 || Math.abs(j2) != j1 || rand.nextInt(2) != 0 && i4 != 0) {
										...
										if (state.getBlock().isAir(state, world, blockpos) || state.getMaterial().blocksMovement()
												...
												|| state.getBlock() == MCreatorCorruptedLeaves.block.getDefaultState().getBlock()) {
											setTreeBlockState(changedBlocks, world, blockpos, MCreatorCorruptedLeaves.block.getDefaultState(), bbox);
										}
									}
								}
							}
						...
						for (int genh = 0; genh < height; genh++) {
							...
							setTreeBlockState(changedBlocks, world, genhPos, MCreatorCorruptedLog.block.getDefaultState(), bbox);
								...
								for (Direction Direction : Direction.Plane.HORIZONTAL) {
									if (rand.nextInt(4 - hlevel) == 0) {
										...
										setTreeBlockState(changedBlocks, world,
												...
												MCreatorCorruptedLeaves.block.getDefaultState(), bbox);
									}
								}
							}
						}
		...
		private boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR || blockType == MCreatorCorruptedLog.block.getDefaultState().getBlock()
					|| blockType == MCreatorCorruptedLeaves.block.getDefaultState().getBlock()
					|| blockType == MCreatorCorruptedGrass.block.getDefaultState().getBlock()
					|| blockType == MCreatorTDirt.block.getDefaultState().getBlock();
		}
