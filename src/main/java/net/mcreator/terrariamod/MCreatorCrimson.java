...
public class MCreatorCrimson extends Elementsterrariamod.ModElement {
	@ObjectHolder("terrariamod:crimson")
	public static final CustomBiome biome = null;

	public MCreatorCrimson(Elementsterrariamod instance) {
		super(instance, 75);
	}
	...
	static class CustomBiome extends Biome {
		public CustomBiome() {
			super(new Biome.Builder()
					...
					.waterColor(-6750157)
					.waterFogColor(-6750157)
					.surfaceBuilder(
							SurfaceBuilder.DEFAULT,
							new SurfaceBuilderConfig(MCreatorCrimsonGrass.block.getDefaultState(), MCreatorTDirt.block.getDefaultState(),
									MCreatorTDirt.block.getDefaultState())));
			setRegistryName("crimson");
	...
	static class CustomTreeFeature extends AbstractTreeFeature<NoFeatureConfig> {
		...
		public boolean place(Set<BlockPos> changedBlocks, IWorldGenerationReader worldgen, Random rand, BlockPos position, MutableBoundingBox bbox) {
			...
			if (position.getY() >= 1 && position.getY() + height + 1 <= world.getHeight()) {
				...
				if (!spawnTree) {
					...
					if (!((ground == MCreatorCrimsonGrass.block.getDefaultState().getBlock() || ground == MCreatorTDirt.block.getDefaultState()
							.getBlock()) && (ground2 == MCreatorCrimsonGrass.block.getDefaultState().getBlock() || ground2 == MCreatorTDirt.block
					...
					if (position.getY() < world.getHeight() - height - 1) {
								...
								for (int i2 = position.getZ() - j1; i2 <= position.getZ() + j1; ++i2) {
									...
									if (Math.abs(position.getX()) != j1 || Math.abs(j2) != j1 || rand.nextInt(2) != 0 && i4 != 0) {
										...
										if (state.getBlock().isAir(state, world, blockpos) || state.getMaterial().blocksMovement()
												...
												|| state.getBlock() == MCreatorCrimsonLeaves.block.getDefaultState().getBlock()) {
											setTreeBlockState(changedBlocks, world, blockpos, MCreatorCrimsonLeaves.block.getDefaultState(), bbox);
										}
									}
								}
							}
						...
						for (int genh = 0; genh < height; genh++) {
							...
							setTreeBlockState(changedBlocks, world, genhPos, MCreatorCrimsonLog.block.getDefaultState(), bbox);
								...
								for (Direction Direction : Direction.Plane.HORIZONTAL) {
									if (rand.nextInt(4 - hlevel) == 0) {
										...
										setTreeBlockState(changedBlocks, world,
												...
												MCreatorCrimsonLeaves.block.getDefaultState(), bbox);
									}
								}
							}
						}
		...
		private boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR || blockType == MCreatorCrimsonLog.block.getDefaultState().getBlock()
					|| blockType == MCreatorCrimsonLeaves.block.getDefaultState().getBlock()
					|| blockType == MCreatorCrimsonGrass.block.getDefaultState().getBlock()
