...
public class MCreatorSkyIsland extends Elementsterrariamod.ModElement {
	public MCreatorSkyIsland(Elementsterrariamod instance) {
		super(instance, 22);
	}
			...
			public boolean place(IWorld iworld, ChunkGenerator generator, Random random, BlockPos pos, NoFeatureConfig config) {
				...
				if ((random.nextInt(1000000) + 1) <= 3500) {
					...
					Template template = ((ServerWorld) iworld.getWorld()).getSaveHandler().getStructureTemplateManager()
							.getTemplateDefaulted(new ResourceLocation("terrariamod", "skyisland"));
					if (template == null)
						return false;
