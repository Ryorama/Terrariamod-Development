...
public class MCreatorTForest extends Elementsterrariamod.ModElement {
	@ObjectHolder("terrariamod:tforest")
	public static final CustomBiome biome = null;

	public MCreatorTForest(Elementsterrariamod instance) {
		super(instance, 15);
	}
		...
		public CustomBiome() {
			super(new Biome.Builder()
					...
					.surfaceBuilder(
							...
							new SurfaceBuilderConfig(MCreatorTGrass.block.getDefaultState(), MCreatorTDirt.block.getDefaultState(),
									MCreatorTDirt.block.getDefaultState())));
			setRegistryName("tforest");
	...
	static class CustomTreeFeature extends AbstractTreeFeature<NoFeatureConfig> {
		...
		public boolean place(Set<BlockPos> changedBlocks, IWorldGenerationReader worldgen, Random rand, BlockPos position, MutableBoundingBox bbox) {
			...
			if (position.getY() >= 1 && position.getY() + height + 1 <= world.getHeight()) {
				...
				if (!spawnTree) {
					...
					if (!((ground == MCreatorTGrass.block.getDefaultState().getBlock() || ground == MCreatorTDirt.block.getDefaultState().getBlock()) && (ground2 == MCreatorTGrass.block
							.getDefaultState().getBlock() || ground2 == MCreatorTDirt.block.getDefaultState().getBlock())))
						return false;
					...
					if (position.getY() < world.getHeight() - height - 1) {
								...
								for (int i2 = position.getZ() - j1; i2 <= position.getZ() + j1; ++i2) {
									...
									if (Math.abs(position.getX()) != j1 || Math.abs(j2) != j1 || rand.nextInt(2) != 0 && i4 != 0) {
										...
										if (state.getBlock().isAir(state, world, blockpos) || state.getMaterial().blocksMovement()
												...
												|| state.getBlock() == MCreatorTLeaves.block.getDefaultState().getBlock()) {
											setTreeBlockState(changedBlocks, world, blockpos, MCreatorTLeaves.block.getDefaultState(), bbox);
										}
									}
								}
							}
						...
						for (int genh = 0; genh < height; genh++) {
							...
							setTreeBlockState(changedBlocks, world, genhPos, MCreatorTLog.block.getDefaultState(), bbox);
								...
								for (Direction Direction : Direction.Plane.HORIZONTAL) {
									if (rand.nextInt(4 - hlevel) == 0) {
										...
										setTreeBlockState(changedBlocks, world,
												...
												MCreatorTLeaves.block.getDefaultState(), bbox);
									}
								}
							}
						}
		...
		private boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR || blockType == MCreatorTLog.block.getDefaultState().getBlock()
					|| blockType == MCreatorTLeaves.block.getDefaultState().getBlock()
					|| blockType == MCreatorTGrass.block.getDefaultState().getBlock()
