...
public class MCreatorSkelletronDungeon extends Elementsterrariamod.ModElement {
	public MCreatorSkelletronDungeon(Elementsterrariamod instance) {
		super(instance, 44);
	}
			...
			public boolean place(IWorld iworld, ChunkGenerator generator, Random random, BlockPos pos, NoFeatureConfig config) {
				...
				if ((random.nextInt(1000000) + 1) <= 1000) {
					...
					Template template = ((ServerWorld) iworld.getWorld()).getSaveHandler().getStructureTemplateManager()
							.getTemplateDefaulted(new ResourceLocation("terrariamod", "skelletrondungeon"));
					if (template == null)
						return false;
